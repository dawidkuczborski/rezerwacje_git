...
export default function LoginClient({ onSuccess }) {
  ...
  useEffect(() => {
    ...
    // reaguj na zmiany w innych komponentach
    ...
    try {
      ...
      if (!user.emailVerified) {
        ...
        try {
          ...
          setMsg("📩 Konto nie jest zweryfikowane. Wysłano ponownie link weryfikacyjny.");
        } catch (err) {
          console.error("Błąd ponownej wysyłki weryfikacji:", err);
        }
        await auth.signOut();
        return;
      }
      ...
      if (resp.data.role !== "client") {
        setMsg("❌ To konto nie jest kontem klienta!");
        await auth.signOut();
        return;
      }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center mt-0 mb-1">
          ...
          <span className="px-1 text-sm" style={{ color: styles.subtext }}>
            lub
          </span>
          <div className="flex-grow h-px" style={{ background: styles.border }}></div>
        </div>

        {/* Przyciski Google / Apple */}
